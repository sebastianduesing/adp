    ...
    for index, rowdict in normalized_data.items():
        ...
        for (scorelist, value) in pairs:
            try:
                ...
                pass
    score_data = {}
    for scores in [c_score_list, w_score_list, p_score_list]:
        ...
        working_dict["mean"] = np.mean(scores)
        working_dict["standard_deviation"] = np.std(scores)
        working_dict["variance"] = np.var(scores)
